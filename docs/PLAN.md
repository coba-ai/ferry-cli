...
| A436 | `README.md`'s status section, `e2e/release_config_test.go` | **`go install` works today and the README did not say so.** The repository is public and, since A435, declares its own module path — so `go install github.com/coba-ai/ferry-cli/cmd/ferry@latest` fetches and builds for anyone, with no tag, no cask and no credential. The README said only that nothing was released yet, which is true about Homebrew and false about installing. It now documents the path and what it does *not* give you: `go install` passes no linker flags, so the binary prints `0.0.0-dev` and `contract unknown` — the designed answer for an unstamped build, but it means the binary cannot say which contract it was built against. | The documented install line is the fifth place the module path is written and the one with the shortest path from a reader to a failure, so it is held to `go.mod` alongside the other four (A435). The failure mode is the same one A435 measured and is worth restating because it does not look like a typo: a README naming the old owner does not 404, the download succeeds through GitHub's redirect and `go install` refuses afterwards with a version-constraints conflict. A reader sees a tool that is broken rather than a document that is stale. | four mutants, each killed with a distinct message: the README naming the old owner, the README naming a package that is not `cmd/ferry`, the install line deleted (killed by the floor, which is the one that matters — an undocumented install path is the defect this row exists about), and `go.mod` renamed alone, which is caught by A435's existing stamp check rather than by this one |
