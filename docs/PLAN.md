...
| Named in prose, never as a table row (M70) | 1 |
...
Of the 17 with no verdict, eight are U6's, because U6's PR reports a count
rather than a table (A371); seven are U5's, five of them money-path (A372);
one is M97, which is only half-measurable across the repository split (A373).
They are listed row by row in `docs/MUTATIONS.md` rather than assumed green —
an unreported row is a mutation nobody ran, and M77 is why that distinction is
not pedantry.
