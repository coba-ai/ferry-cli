...
| A400 | §4.1, §8 D-2, §5.14 | The CLI is its own repository, `kurenn/ferry-cli`, module `github.com/kurenn/ferry-cli`. `docs/api/openapi.yaml` and `docs/api/errors.md` are vendored under `contract/`, digested in `contract/SOURCE` and held to those digests by `contract_test.go`. This plan moved to `docs/PLAN.md`. `OWNERSHIP` gives `contract/`, `docs/`, `.github/` and `README.md` to U1 with the rest of the repository scaffolding. | A product decision, not a technical finding: the CLI and the API ship to different people on different clocks. The cost is that the AC85/AC86 pins and the `internal/outcome` catalogue checks now read a copy rather than the generated file. Vendoring keeps them running; the digests make a local edit loud. The remaining gap — upstream changing while the copy stands still — is open, and needs the API repository's CI to reach into this one. | the digest check, shown failing on a one-line edit to `contract/errors.md` before being committed |
